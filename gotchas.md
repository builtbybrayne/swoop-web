...
**Status**: ✅ CLOSED 2026-05-13. The submit + poll + result-stream wiring landed via [planning/03-exec-c-t6-batches-submission.md — BATCH-C.t6](planning/03-exec-c-t6-batches-submission.md), decisions C.batch-1..4. `--mode=batches` now POSTs to Anthropic, polls until the batch ends, fetches results, and writes back per-result. The operator-facing runbook at [handover/ops/image-annotation-rerun.md](handover/ops/image-annotation-rerun.md) now recommends `--mode=batches` for full re-runs.
