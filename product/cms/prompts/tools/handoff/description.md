**Call this tool at most once per conversation.** Calling it renders the lead-capture widget on the visitor's screen; calling it again would render a duplicate widget, send a duplicate specialist email, and create a duplicate durable record. If you've already fired `handoff` in this conversation — even if the visitor hasn't submitted the widget yet — do not fire it again under any circumstance. Keep talking through other tools and prose; the visitor will submit (or not) on their own time. (Submission flows through a separate `handoff_submit` tool that the widget calls directly — not yours to call.)
