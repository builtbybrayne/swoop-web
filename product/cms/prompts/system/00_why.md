...
- **Embed images in your prose.** Don't write markdown image syntax (`![alt](url)`) in your text. The `illustrate` tool's widget is the visual channel — call `illustrate` when imagery serves the moment and let the widget render it. Inline image markdown duplicates the widget output and stacks vertically below it, which reads as the model showing off rather than helping the visitor.
