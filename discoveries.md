...
## 2026-06-19 — `npm run demo` is a production `vite build` (strips every `import.meta.env.DEV` affordance); and native thinking is a per-runner property, not a per-request knob

Two truths from the dev-affordances + thinking-toggle work (decisions DEMO-DEV-1..2, TT-1..6):

**1. Demo mode dead-code-strips dev tooling — gate visible dev affordances through `isDevToolsEnabled()`, not `import.meta.env.DEV`.** `scripts/demo.sh` runs `npm run build -w @swoop/ui` (a production `vite build`) then `vite preview`. Vite compiles `import.meta.env.DEV` to the literal `false` in a production build and dead-code-eliminates the branches — so the model picker, the "Show/Hide dev" toggle, and the widget tool-traces all vanish on the demo server (which is exactly where Luke uses the picker). The "Show dev" toggle is purely client-side and was *also* gone, which isolated the cause to the UI build, not the orchestrator. Fix: a DRY `isDevToolsEnabled()` = `MODE !== 'production' || VITE_SHOW_DEV_TOOLS === 'true'` ([dev-tools.ts](product/ui/src/runtime/dev-tools.ts)), with `demo.sh` baking `VITE_SHOW_DEV_TOOLS=true` into the build. Proven by a build-fold contrast — flag-on → `function isDevToolsEnabled(){return true}`, flag-off → `return false`. Read `MODE` (a string `vi.stubEnv` overrides), not the boolean `DEV` (can't be stubbed cleanly) — same reason [reasoning-guard.ts](product/ui/src/parts/reasoning-guard.ts) keys off MODE. **Gate split**: only *visible* affordances move to the helper; genuine dev-only guards (the reasoning-leak throw, preflight/rehydrate console logging) stay on `import.meta.env.DEV` — you don't want those firing on the demo box.

**2. Native thinking is baked into the agent (cached prefix + request shape), so toggling it needs a runner variant — it can't be a pure per-request param.** Flipping thinking changes two things constructed in `buildOrchestratorAgent`: the `ClaudeLlm` `thinking` fragment AND the RL.3 silent-working belt, which is spliced into the system prompt ONLY when thinking is off ([factory.ts](product/orchestrator/src/agent/factory.ts)) — and the system prompt is the cache-stable prefix. So a thinking override resolves a distinct agent/runner; the dev thinking toggle therefore rides the M-PICK runner-registry with the cache key widened to `(model, thinking)` rather than tweaking a request field. `effort`, by contrast, IS a pure per-request param (not in the prefix) — which is why the greeting pre-warm drops effort to `'low'` while inheriting the conversation's thinking (index.ts). **Rule**: when adding a "flip X per session" dev control, first ask whether X lives in the cached system prefix — if yes → runner variant; if no → request field.

## 2026-06-17 — A strip-the-reasoning pipeline was built but thinking was never enabled — the producer/consumer split is a false-completeness trap

B.13 re-architected reasoning isolation from `<reasoning>`/`<utter>` tags to native Anthropic thinking blocks. The **consumer** half shipped and looked complete: [reasoning-filter.ts](product/orchestrator/src/translator/reasoning-filter.ts) strips reasoning parts off the SSE wire, and [claude-llm.ts](product/orchestrator/src/agent/claude-llm.ts) maps `thinking_delta` → `thought:true`. Tests were green, the strip demonstrably worked — so the feature read as done. But the **producer** half was never wired: `thinking` was never set on the `messages.create` request, and the system prompt carried no output discipline. With no thinking channel, the model simply reasoned in plain `text` — the "responses got longer / narrate their planning" leak. **Lesson**: a consumer that correctly handles a producer's output is not evidence the producer is emitting anything. When a capability is split across request-shaping and response-handling, the only proof it's live is an end-to-end check that the *request* carries the feature flag — green strip-side tests prove the strip, not the source. Grep for where the request is built (`messages.create` params), not just where the response is parsed.

**Phase-1 smoke outcome — the signature round-trip was NOT needed.** The open architectural question was whether Anthropic requires the thinking block (with its `signature`) to be replayed in the assistant turn that carries a `tool_use` when tool results are sent back — the current code drops thinking on replay. A single live smoke (session `0fee8d42`, thinking on, sonnet-4-6) settled it: the full tool loop `list_skills → load_skill → find_inspiring → illustrate → answer` completed with **no 400** on any post-tool-result continuation, while server-side `reasoningCount` was 28 (thinking genuinely active) and **zero** reasoning frames reached the wire (strip intact). So the speculative, complex signature-capture-and-replay path (Phase 2 of the plan) was **skipped** — don't build it unless a future model/path actually 400s for a missing pre-`tool_use` thinking block. One number to watch: that smoke turn was ~23s end-to-end; `ORCHESTRATOR_EFFORT=medium` is the latency lever if `high` (the default) proves too slow for the discovery chat.
...
**Third instance — 2026-06-11 filter-sparsity hot patch**: the same class bit two more primitives at once. `find_inspiring`'s `region`/`mood` filters (`inspire_passage.region` + `.mood`, 0/665 each — compose pass never wrote them) zeroed both hybrid CTE legs. `illustrate`'s `regionSlug` filter (`image.region_tags @>`, 0/13,012 — Vision reminder bug, parked) returned zero rows whenever the agent supplied a slug; and `cms/prompts/tools/illustrate/description.md` line 3 was actively inviting the model to pass one. `find_options` hotel branch: `accommodationStyle` ILIKE on `hotel.description` (0/44 populated) similarly guaranteed zero results. Fix: drop the four zero-guaranteed SQL clauses; accept fields in schema/options (removing them would cause input_validation rejections) with accepted-but-ignored comments; update tests to assert accepted-not-forwarded. **Extended pattern rule**: never expose a hard filter over a column whose population you haven't probed with a coverage query. Zero-population columns are not "empty today, useful tomorrow" — they're silent result-killers that look like bugs in the agent, not in the data. Tier-3 plans for any new retrieval primitive must include the coverage probe before implementation (not just before ship). Full execution log at [planning/03-exec-c-t4.md — 2026-06-11 filter-sparsity hot patch](planning/03-exec-c-t4.md).
