...
Luke's 16/06 feedback (agent lacks current sales knowledge — Patagonian seasonality / refugio availability — and mis-reasons season-vs-date) is being addressed two ways. (1) **Now, independent**: a static always-on `cms/prompts/system/20_field-notes.md` seeded with the seasonal facts currently *trapped* in conditionally-loaded skills (`arrived-with-ai-itinerary`, `engaging-a-planner`, `group-tour-surfacing-for-solos`, `pattern-budget-solo-traveller`) + the Southern-Hemisphere season-inversion anchor drafted in the 2026-05-18 entry below (still unshipped — this finally lands it), under an authoritative-framing header. **Accepted & live 2026-06-18** (DRAFT banner removed from the file; not a draft). (2) **The general capability**: inline sales-authored agent memory — [planning/02-impl-sales-memory.md](planning/02-impl-sales-memory.md) (DRAFT T2 + `sm-*` decisions). The 2026-05-18 seasonality-anchor note below is effectively superseded by (1).
...
**RESOLVED same day** (fix committed on the styling worktree branch): the cap was R4 abuse-bounding from the 2026-04-30 review, mis-calibrated after the frosty-leavitt form polish routed the rich `specialistSummary` through `reasonText`. Fix: shared constants in `ts-common/src/handoff.ts` (`HANDOFF_NARRATIVE_TEXT_MAX = 2_000`, matching the motivationAnchor/additionalNotes narrative class), enforced FIRST at the agent tool boundary (`tools.ts` — over-budget fails the tool call, agent rewrites, recoverable) and mirrored on wire + payload + widget. Also closed the same latent trap on `motivationAnchor` (was uncapped on the wire, capped on the payload). Verified live: a 700-char submit that previously rejected now lands `ok:true`; a fresh conversation produced a 1,934-char organic summary that flowed through to the durable record.
