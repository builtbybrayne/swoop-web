...
a. **`customerreview` and `customertip` source tables are missing from the SQL dump.** The junction tables `contentblock_customerreview` (2,390 rows) and `contentblock_customertip` (119 rows) carry FK constraints pointing at `customerreview.id` and `customertip.id`, but those source tables don't exist in the export. So the prose content of customer reviews/tips is absent. **Question**: are those tables intentionally excluded from the export (e.g. PII), or is the data elsewhere (separate CMS, deleted, etc.)? Without the prose, the agent's `recall_someone_who` and `build_confidence` tools fall back to blog posts.
