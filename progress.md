...
| **C — retrieval & data** | MCP connector + Postgres derived store + ETL + sales-shaped composer tools + annotation pipeline | Tier 2 rewrite landed 2026-04-28; Tier 3 plans pending | Stub connector in orchestrator's `test-fixtures/` carries M1. SQL dump arrived 2026-04-27 (`/data/content-data-swoop-patagonia_prod.sql`); Julie call closed design questions; chunk C Tier 2 fully rewritten in [planning/02-impl-retrieval-and-data.md](planning/02-impl-retrieval-and-data.md). C.t2 (sales-shaped tool I/O + Postgres entity model) is the live HITL thread at [planning/00-discovery-design-thinking.md](planning/00-discovery-design-thinking.md). |
