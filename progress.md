...
**Remaining — Alastair's editorial domain (G.7)**: T3-5 voice — now plain markdown, **no code change**: edit `cms/prompts/memory/mode-wrapper.md` + `loaded-header.md` (+ `tools/*.md`), preserving the load-bearing structural signals; the `20_field-notes.md` voice pass (the static stopgap, DRAFT `3ea28ee`); and a `luke-` harness run to confirm the seasonal-knowledge behaviour. One noted optimisation (non-blocking): the memory agent re-seeds a fresh session per memory turn (works via `conversationHistory` accumulation — sm-9; folding the transcript into the first message is the future refinement).
