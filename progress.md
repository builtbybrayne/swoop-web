...
**Remaining — Alastair's editorial domain (G.7)**: T3-5 voice — two `TODO(T3-5)` placeholder headers (the memory-mode wrapper in `memory-agent.ts`; the `AUTHORITATIVE_HEADER` in `sales-memory-loader.ts`), each carrying load-bearing structural signals to preserve; the `20_field-notes.md` voice pass (the static stopgap, DRAFT `3ea28ee`); and a `luke-` harness run to confirm the seasonal-knowledge behaviour. One noted optimisation (non-blocking): the memory agent re-seeds a fresh session per memory turn (works via `conversationHistory` accumulation — sm-9; folding the transcript into the first message is the future refinement).
