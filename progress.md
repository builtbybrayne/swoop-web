...
- **T3-1 store+CRUD** (migration 020, `@swoop/common` schemas, connector data layer + 5 memory tools) + **T3-2 staff auth** (`StaffAuthenticator` shared-password+JWT, `/staff/auth`, session staff/mode flags, additive `staffToken`) — two parallel worktree agents, merged (`e8faaa4`, `3c54710`).
- **T3-3 routing + Opus memory agent** (`a7a29ed`) — orchestrator-level mode routing (sm-2), the Opus memory agent in a transcript-seeded **isolated** session (sm-9), token-bound CRUD to the connector hard-reject seam (sm-4), `finish_memory` handback + a **hard user-exit backstop**. A mandatory live Anthropic smoke (Opus 4.8) **caught two real bugs the 41 mocked-LLM tests all missed**: (1) the `ClaudeLlm` shim sent `temperature` unconditionally → 400 on Opus 4.7+/Fable (sampling params removed there) — shim now model-aware, Sonnet/Haiku byte-identical; (2) the memory tools leaked the auto-injected `author`/`staffToken` into the agent-facing schema → the agent asked staff for an author instead of saving — now stripped. Both logged in [gotchas.md](gotchas.md).
- **T3-4 per-turn loading** (`ed8e87a`, sm-6) — the conversational agent's `InstructionProvider` reads the active set from the connector each turn (E.11; never a direct orchestrator PG query) and splices an authoritative, timestamped, **byte-stable** block into the system prompt for **every** session. Closes Luke's loop: authored seasonal knowledge reaches every visitor, stated as fact, dated for staleness reasoning.

**Verified**: typecheck clean; full suite green at the tip — orchestrator **310** (+18 exit-trigger tests in T3-3, +21 loader tests in T3-4), connector 232, ts-common 243, ui/cms/harness unchanged; **live Opus-4.8 smoke PASS** (confirm→store→`finish_memory`, multi-turn context intact). Commits `e8faaa4` `3c54710` `a7a29ed` `ed8e87a` on the feature branch (architecture lives in decisions `sm-1`…`sm-9`).

**Prompts relocated to CMS (`5ec30a9`)**: the T3-3/T3-4 build had hardcoded ~14 prompt strings (memory-mode wrapper, loaded-block header, seed preamble, memory tool descriptions — the last duplicated connector+orchestrator). All moved to `cms/prompts/memory/` + loaded fail-fast at boot (per the content-as-data rule), the duplication collapsed to a single source, +4 deterministic loader tests, behaviour byte-preserved. Verified counts now orchestrator **312** / connector **234** / ts-common 243.

**Remaining — Alastair's editorial domain (G.7)**: T3-5 voice — now plain markdown, **no code change**: edit `cms/prompts/memory/mode-wrapper.md` + `loaded-header.md` (+ `tools/*.md`), preserving the load-bearing structural signals; and a `luke-` harness run to confirm the seasonal-knowledge behaviour. (`20_field-notes.md` is **accepted & live** as of 2026-06-18 — DRAFT banner removed; not a draft, do not re-flag.) One noted optimisation (non-blocking): the memory agent re-seeds a fresh session per memory turn (works via `conversationHistory` accumulation — sm-9; folding the transcript into the first message is the future refinement).
...
- **Compose pass** ✅ — `inform_chunk` 924 → **1,224** (Guidebook class gap closed; costs page `/visit/costs` now 8 embedded chunks), 0 lorem leaks, `trip_card` zero-prices 8 → 0, 321 fresh embeddings at £0.005.
- **Live-stack smoke** ✅ — boot: 11 tools / 10 exposed incl. `get_pricing` (the merge-time TOOL_SPECS fix verified live). **Activity status line worked to spec on first run**: one slot, per-tool copy swapping ("Browsing trip ideas…" → "Finding a picture…" → "Checking Swoop's guides…"), cleared on text. Turn 2 of a kayaking+pricing conversation fired **find_options → show_options → get_pricing → illustrate** in order; browse rendered silently (dev note confirms), show rendered 4 curated cards; reply carried real hotel pricing with package-vs-per-night arithmetic shown ("$3,600 for 3 nights — roughly $1,200/night"), the April-2026 as-of caveat, and the Specialist-confirms line. Bonus verifications: browser-timestamp dateline ("March 2027 is nine months out"), terminology card, costs-page link with title anchor.
- **Harness** — `tool_call_order` assertion kind added (`619bf39`, +5 tests; the reshape log had flagged it missing). Accidental full scripted-suite sweep at the merge tip: **14 passed / 0 failed** (39 agent-as-user scenarios error without ANTHROPIC_API_KEY in the harness env — export it from orchestrator/.env when running those). Scoped `agent-212-kayaking-relevance` run **failed ×2**: agent browses but never calls `show_options`. Root cause round 1: two skills still taught `find_options` as the visual surface + the show step read as optional — fixed (`1bd4569`: planner + group-tour skills teach the two-step; both descriptions make same-turn show the SHOULD-default). Round 2 (post-fix) re-run still fails, but the transcript shows defensible behaviour: the simulated visitor is an expert with a hyper-specific ask (Tortel-channels expedition kayaking), the agent browsed 2×, was honest that it's "not a catalogue product", kept expert prose warmth, and handed off when the visitor demanded a specialist. **Open calibration question for Alastair's editorial pass**: should the agent show near-fit cards (Kayak Pumalin Fjords etc.) as anchors *before* conceding the catalogue limit, or is honest prose right for expert-specific asks? Scenario stays failing (non-gating by design) until that call. Transcripts: `product/harness/runs/2026-06-12T10-24-25Z/`.
- **Ops gotcha recorded** (gotchas.md): the Claude Preview harness injects `PORT` into managed processes — knocked the orchestrator off :8080 inside the concurrently stack; launch config now strips it with `env -u PORT`.

Residual operator items: Mini-side update (it still runs the pre-wave build), Luke email (luke-briefing.md has the six items), editorial passes (tool-status copy, pricing §5 wording, the show-near-fits calibration above).

## 2026-06-11 — Pricing-data research + find_options reshape: two T3 plans authored (HITL session, worktree `goofy-goldstine-2ed1c1`)

Research on Alastair's "can we get price data in from the web data we have?" question, widened mid-session into the find_options relevance gap. Headline findings (all probed live against `puma_dev` + the source dump): **trip prices were never removed** (C.14 kept headline `base_price`; 464/649 trip cards carry `from_price`, displayed today); the 2026-04-27 dump contains a **current-season (2025/26) hotel pricing matrix the ETL never loads** (26/44 hotels, 1,051 seasonal price points across 108 seasons + 97 room types — [query-hotels.ts](product/connector/src/data/query-hotels.ts) already consumes `MIN(hp.price)`, so the Explora-on-a-budget-query case is literally an empty table); **`find_options` uses no embeddings** (pure SQL filter + `ORDER BY RANDOM()` per C.t4 — prime suspect for Luke's D5 relevance feedback, while `trip_card.embedding` + `tsv` sit idle); the canonical costs page `/visit/costs` has **0 `inform_chunk` rows** so `lookup`-steered cost questions hit the stale undated FAQ instead; source hotel prices are **1–6-night packages** (a naive load would mislabel them per-night — wrong by up to 5×). Prompt-side state of play confirmed for Alastair: Puma's prompts have *never* barred price talk (cost-bands MAY + "I can't help with pricing" explicitly named wrong phrasing since the 14 May authoring); the don't-talk-about-price memory traces to the Phase-1 PoC's `guidance-payload.json` (pricing was a handoff trigger there).

Two DRAFT crosscut plans authored, key decisions pre-ratified in-session (each plan carries a HITL ratification appendix):

- **[03-exec-crosscut-goofy-goldstine-pricing-data.md](planning/03-exec-crosscut-goofy-goldstine-pricing-data.md)** — hotel-pricing faithful flatten (`nights` kept raw; new migration), hotel prose carry (incl. `what_we_dont_like`), per-night card prices + per-night-scale budget ceilings, `get_pricing` raw-matrix tool (tenth) stamped `capturedAt` (config, default 2026-04-27 — **no fresh dump available**), §5 staleness guardrails (as-of awareness, once-per-turn dynamic-prices line, top-end generosity), costs-page → `inform_chunk` fix + lorem-ipsum guard. **Merges first** (tool-count pins).
- **[03-exec-crosscut-goofy-goldstine-find-options-reshape.md](planning/03-exec-crosscut-goofy-goldstine-find-options-reshape.md)** — phase 1: optional `query` param + hybrid (embedding+tsv) ranking for trips/tours via the existing `buildHybridSearchSql`, coverage-probed filters stay, two live tour traps die, `RANDOM()` → tiebreak/fallback; phase 2: find/show split (`find_options` = agent-private compact browse, renders nothing; **`show_options`** (Alastair's name, eleventh tool) = curated full cards, seen-marking moves to it, trip/tour no-dedupe carve-out preserved); phase 3: `also_interesting` compact strip as an **isolated UI-only commit** for the parallel styling agent.

### Same-session execution wave (ratified mid-session; third plan added)

Alastair ratified in-conversation and added a third plan ([activity-status](planning/03-exec-crosscut-goofy-goldstine-activity-status.md) — ephemeral tool-derived "what the agent is doing" line beside the thinking dots, extending the D.10 fyi channel; plus [luke-briefing.md](luke-briefing.md), a new root capture file for business implications, seeded with six entries and documented in CLAUDE.md). Three Sonnet background agents dispatched in parallel worktrees; a mid-wave harness restart pruned two of the three worktrees, so the activity-status and reshape agents finished **inside the orchestrator's worktree** (clean commits; lesson logged in discoveries.md 2026-06-11). The pricing agent's planning leg landed in the **main-repo working tree** (wrong-cwd class) — content recovered into `b226713`; stray cleanup on main left for Alastair.

**All three merged on `claude/goofy-goldstine-2ed1c1`** (`faddb11` pricing → `18471e5` activity-status → `25222d2` reshape): hotel pricing live in puma_dev (1,051 rows, 26/44 hotels; Explora correctly excluded from budget queries, EcoCamp/AWA pass), `get_pricing` + `show_options` join the tool surface (**union = 11 tools / 10 exposed**), hybrid RRF ranking on find_options (live probe: "sea kayaking among glaciers and fjords" → kayaking trips top-ranked — Luke's D5 relevance gap visibly closed), find/show split (browse renders nothing; Sonnet performed the two-step unprompted 3/3 in the real-Anthropic smoke), Guidebook pagetype class-gap fix (86 guide pages incl. the costs page enter `inform_chunk` on next compose), §5 staleness guardrails landed.

**Merge-time regression caught**: the pricing branch registered `get_pricing` connector-side but never added it to the orchestrator's `TOOL_SPECS` — the model could not call it (the find_tips regression class). Fixed in merge commit `25222d2`. Sibling trap: both branches changed identical count-pin lines 9→10, which git auto-merges silently where the union needs 11 — see discoveries.md 2026-06-11.

**Operator-pending after merge**: compose pass with GEMINI_API_KEY (loads the 86 guide pages + refreshes the 8 stale `trip_card` zero-prices), real-Anthropic `get_pricing` smoke, live widget/status-line smokes, harness `agent-212-kayaking-relevance` run, main-repo stray cleanup, editorial pass on `cms/ui/tool-status.en.json` copy. **Fresh-install verification at the tip (rm -rf node_modules + npm install): GREEN** — common 202 / orchestrator 220+21sk / connector 211+5sk / ui 216 / ingestion 300 / harness 200 = **1,349 passed + 26 skipped** (was 1,303+26 on main → +46 net); typecheck clean across all six workspaces. Branch ready for Alastair's merge to main.
...
### Same-day execution wave (evening)

Alastair ratified ("postgres sessions asap … then proceed"); B.t13 was promoted to a full plan ([03-exec-agent-runtime-t13.md](planning/03-exec-agent-runtime-t13.md)) and the whole wave ran as a 7-agent Sonnet background swarm (one worktree each, hash-gated) + B.t12 staged behind B.t13's merge (shared `chat.ts`/config surface). **All eight merged to `main`** (`879d0d7` handoff-form → `bdc5272` content → `abe299b` B.t13 → `624cb56` terminology-card → `343d1e2` demo-scripts → `f56834c` visual-channel → `001f87b` B.t12 → `727c4ac` provenance). Fresh-install verification green at tip: **1,303 passed + 26 skipped across all six workspaces**. Orchestrator-side merge fixes: phantom `supertest@^7.12.0` revert (`e28e9f3`), migration-manifest test pin (`90211e2`), one predicted `migrate.test.ts` conflict resolved to the union. Wave findings worth knowing: Luke's stale price lives in an **undated FAQ row** (not the blog — source-correction ask added to questions.md); the **blog corpus is absent from `puma_dev`** (blog-date provenance idle until re-ingest; chip raised); **`hotel_pricing` is empty** (budget filter no-ops on hotels — the Explora-on-a-budget-query case, reinforcing the Product-Library deferral). Four of the five stalled-agent recoveries this wave came from the `"stuck?"` nudge protocol. Residual operator steps: ledger checklist ⚠ marks (editorial pass on `b90ff8b..e17d7ba`, live smokes, Mini install + B.t13 restart smoke + Part A investigation, card-copy sign-off).
...
- Operator runbook ([handover/ops/image-annotation-rerun.md](handover/ops/image-annotation-rerun.md)) flipped from "deferred" to "preferred for full re-runs (~$17/£14 vs ~$34/£27 at live rate)".
...
- `handover/ops/sync-mode.md`: operator runbook entry for `--sync`.
...
| [planning/03-exec-c-t8.md](planning/03-exec-c-t8.md) | 235 | (same) | ETL + annotation handover runbooks at `handover/ops/` |
