...
- **B.t3a — Connector adapter augment** (post-M1, gated on chunk C): extends `product/orchestrator/src/connector/tools.ts` with ADK FunctionTool wrappers for the 5 new sales-shaped tools introduced in [02-impl-retrieval-and-data.md](02-impl-retrieval-and-data.md) §2.2 (`stoke_imagination`, `offer_options`, `recall_someone_who`, `build_confidence`, `compare_paths`). Existing wrappers for the 5 PoC-derived tools (`search`, `get_detail`, `illustrate`, `handoff`, `handoff_submit`) stay in place. Validates against new Zod schemas in `@swoop/common`. **Gated on**: C.t2 (new schemas land in `ts-common`) + C.t4 (real connector implementations exist). **Not blocking M1** — the existing 5-tool surface is sufficient for the vertical slice.
