...
- **Session state via ADK's native `SessionService`** (in-memory for Phase 1 — the simplest in-built ADK option that works; post-M4, the upgrade path is a **custom Postgres `SessionService` implementation** writing to our Cloud SQL instance, if budget allows). Vertex AI Session Service no longer in scope; Firestore dropped per C.23.
...
- Session state (conversation history + triage state + wishlist-in-progress) is accumulated, queryable, and persisted behind a `ts-common` `SessionStore` interface. Phase 1 implementation: in-memory + ADK-native (`memory://`). Production-backend stub (Postgres-backed `SessionService`) sits behind the interface, throwing "not implemented" until post-M4. **Note**: B.t2 shipped against the original plan with a `firestore.ts` interface stub; that file is renamed/replaced as `postgres.ts` as part of the C.23 Firestore cleanup — captured as a follow-up.
...
Google ADK provides a `SessionService` abstraction with extensible backends — in-memory (default), database-backed (custom impls slot in), and a couple of pre-built integrations. **Puma leans on ADK's session-service shape rather than rolling our own persistence layer.** Phase 1 uses ADK's in-memory service — the simplest in-built option that works for the vertical slice. When persistence becomes necessary (post-M4), the upgrade is a **custom Postgres `SessionService` implementation** writing to the same Cloud SQL instance that holds the retrieval and handoff stores (per C.18, C.23). The choice is exposed behind a thin `ts-common` interface so the orchestrator code stays agnostic.

Why a custom Postgres impl rather than something pre-built: (a) we already have Cloud SQL Postgres running for retrieval + handoff — adding a third store would be a single-store-philosophy violation; (b) a thin custom impl is days of work; (c) Vertex AI Session Service is no longer in scope (cost, lock-in, and a separate ops surface for marginal benefit); (d) Firestore was the previous default but is dumped per C.23. Decision B.22 — see [decisions.md](decisions.md).
...
| B.2 | Session backend strategy | **ADK's native `SessionService`.** In-memory for Phase 1 (simplest in-built option that works). Production: a **custom Postgres `SessionService` implementation** writing to our Cloud SQL instance post-M4 if budget allows. Vertex AI Session Service no longer in scope. Firestore dropped per C.23. Exposed behind a thin `ts-common` interface. See B.22 in [decisions.md](decisions.md). | Use ADK's session-service shape to avoid reinventing persistence; defer the implementation until post-M4 when usage patterns are visible. Single-store philosophy via Cloud SQL Postgres (sessions + retrieval + handoff in one DB) — operationally simpler for Swoop's team. |
...
- **Custom Postgres `SessionService` implementation shape** (post-M4): table layout (one row per session vs append-only event log), index strategy for active-session lookup, idle-archive sweeper invariants, schema-evolution path for new state fields. The B.22 decision pins Postgres as the destination; the implementation specifics are Tier 3 work.
...
- **B.t2 — Session interface + ADK in-memory adapter**: thin `ts-common` `SessionStore` interface over ADK's `SessionService`; in-memory + ADK-native (`memory://`) implementations for Phase 1; **stub/signature for the future Postgres `SessionService` implementation** (post-M4 wiring). **Note**: this task originally shipped with a `firestore.ts` interface stub; that file is renamed/replaced as `postgres.ts` as part of the C.23 Firestore-cleanup follow-up. The `SESSION_BACKEND` env enum value `"firestore"` becomes `"postgres"` in the same pass.
...
- **B.t3a — Connector adapter replace** (post-M1, gated on chunk C; revised 2026-04-29): rewrites `product/orchestrator/src/connector/tools.ts` to register ADK FunctionTool wrappers for the eight intent-named tools settled in [02-impl-retrieval-and-data.md](02-impl-retrieval-and-data.md) §2.2 + decision C.25 (`find_inspiring`, `find_someone_who` *(conditional on C.26)*, `find_proof`, `lookup`, `find_options`, plus carried-forward `illustrate`, `handoff`, `handoff_submit`). The five PoC-derived wrappers collapse: `illustrate`/`handoff`/`handoff_submit` survive intact; `search` and `get_detail` are deprecated alongside their tools (their surface absorbs into `lookup` / `find_options`). Validates against new Zod schemas in `@swoop/common`. **Gated on**: C.t2 (new schemas land in `ts-common`) + C.t4 (real connector implementations exist). **Not blocking M1** — the existing PoC tool surface continues to back the vertical slice until C.t1+C.t4 land. Earlier (2026-04-28) phrasing of this task as an *augment* — extending alongside the PoC tools rather than replacing them — is superseded; the 2026-04-29 surface (eight intent-named tools, no composer) deprecates `search`/`get_detail`.

Parallelism: B.t1, B.t2, B.t6 proceed in parallel within a single agent's session. B.t3 waits on chunk C's connector existing (stubbed is fine). B.t4 + B.t5 are adjacent — one agent does both, or two agents split cleanly. B.t7 is the integration — single-agent. B.t8 is conditional. B.t9 pairs with G.t3. B.t10 is a deliberate post-M1 task. **B.t3a runs in parallel with D.t9** (chat-surface widget augment) once chunk C's tool surfaces land.

Estimated: 3–4 days of focused work for B.t1–B.t7 (vertical-slice scope), single agent, once chunk A's foundations land. B.t8–B.t10 add 1–1.5 days depending on Phase 1 spike outcomes. **B.t3a is mostly mechanical** — adds 5 wrapper functions + 5 Zod schema imports + matching test fixtures; ~0.5–1 day once C.t2 + C.t4 are in.
