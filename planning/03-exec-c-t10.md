...
- Documentation update in `handover/ops/embedding-rerun.md` (or wherever the operator runbook covers enrich) noting the new mode + when to use it.
...
| `handover/ops/embedding-rerun.md` *(or new ops doc)* | Operator runbook entry: when to reach for `--sync`, cost expectations, what it doesn't do. |
...
Add a new ops doc at `handover/ops/sync-mode.md` or append to the existing enrich runbook (`handover/ops/embedding-rerun.md` — verify which file is most relevant; the C.t8 runbooks should cover this surface area):
...
- **Operator runbook**: [`handover/ops/`](../handover/ops/) (C.t8 deliverable; gains a sync-mode entry).
...
| 6 — operator runbook | ✅ done (commit `fa0b733` — `handover/ops/sync-mode.md`) |
