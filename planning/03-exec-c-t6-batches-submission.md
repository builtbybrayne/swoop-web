...
- `handover/ops/image-annotation-rerun.md` — **edit** — flip the operator caveat from "use --mode=live; --mode=batches is unwired" to "--mode=batches now submits + polls + writes back; --mode=live remains supported for small slices".
...
[handover/ops/image-annotation-rerun.md](../handover/ops/image-annotation-rerun.md) — flip the section that warns about `--mode=batches` not being wired. New language: "`--mode=batches` submits via Anthropic's Batches API (50% discount, up to 24h SLA). Use for full-corpus runs; `--mode=live` remains the supported path for small slices during prompt iteration."
...
- `handover/ops/image-annotation-rerun.md` — operator runbook to update.
...
3. **Docs + decisions**: operator runbook ([handover/ops/image-annotation-rerun.md](../handover/ops/image-annotation-rerun.md)) flipped from "deferred" to "preferred for full re-runs". `gotchas.md` "annotate-images --mode=batches" entry rewritten as closed-historical. Decisions C.batch-1..4 appended to `planning/decisions.md`.
