...
- **E.t5 — Disclosure + consent copy authoring**: open. `product/cms/legal/` content authoring is Al's editorial work. Today's strings are placeholders inline in the components.
  - **Copy-versioning gap to close before launch**: today's `CONSENT_COPY_VERSION = "consent-handoff/v1"` (in `product/ui/src/widgets/lead-capture.tsx`) and the orchestrator's `disclosureCopyVersion = "v1"` default (in `product/ui/src/disclosure/use-consent.ts`) are manual semver-style labels. They must be bumped by hand whenever consent-bearing copy changes, with no enforcement. The intended state before public launch is **content-hash versioning** — the version id becomes a hash of the literal copy strings, so any edit automatically invalidates the prior version with no human coordination. Surfaced to counsel as **D-3.2.4** in the legal review pack (`planning/swoop-legal-review-pack.md`); flagged here so the gap is tracked alongside E.t5 copy authoring. Implementation likely lands in the same workspace touch as E.t5 (i.e. when copy moves from inline React strings into `product/cms/legal/`).
