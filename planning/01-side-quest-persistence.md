...
- **Theme 2 — Content-as-data, extended to state.** Conversation state lives server-side (ADK session), not in the browser. The client persists a **reference** (session id), not the data. This keeps the single source of truth on the server — good for later observability, good for consent compliance (deletion hits one place), good for the eventual durable-session migration (custom Postgres `SessionService` per B.22 + C.18; Firestore was the original target but is dropped per C.23).
