...
9. Spot-check runbook (Tier 2 §2.5) — owned by F.t4, a separate deliverable. F-b leaves a one-paragraph note in `handover/ops/` (or defers the note entirely to F.t4) confirming the happy-path sequence above. Not a blocker for F-b completeness; noted so F.t4's author knows what to write against.
