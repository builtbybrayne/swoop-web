...
**Decision**: name the carve-out explicitly. `--mode=live` is the supported full-run path. `--mode=batches` is **request-build-verified only** until the submit + poll + result-stream wiring is built. The follow-up task is now queued in [next-steps.md — Chunk C section](../next-steps.md) and the operator-facing gotcha is at [gotchas.md — annotate-images batches](../gotchas.md). The original runbook caveat at [handover/ops/image-annotation-rerun.md](../handover/ops/image-annotation-rerun.md) stays as the read-this-before-running gate.
...
**Swap cost**: Trivial. One sentence in `handover/ops/evalset-growth.md` § "Cadence + ownership" gets updated whenever ownership changes.
