...
- **RL.8** — Re-baselined the `luke-01…12` judged harness family after the thinking change (thinking shifts outputs; the prior 5/12 judged baseline must be re-measured). **Result**: _pending — recorded at close of this session; compared against [luke-baseline-judged-2026-06-12](../product/harness/runs/luke-baseline-judged-2026-06-12/results.md) (main checkout)._ *Swap cost: n/a — measurement only.*
...
**Decided**: 2026-06-11 (plan ratified in the goofy-goldstine HITL session) → **2026-06-11 (built)**
**Owner**: activity-status execution session ([planning/03-exec-crosscut-goofy-goldstine-activity-status.md](03-exec-crosscut-goofy-goldstine-activity-status.md) §2.2/§3 — execution log at the bottom of that plan)
**Rationale**: Long tool calls left the chat column showing nothing once widget loaders moved to the visual sidebar. assistant-ui `tool-call` parts already stream their lifecycle (`input-available` before `output-available`), so "what's happening right now" is derivable entirely client-side — **no orchestrator, wire, or schema change**. The thinking-dots component ([text-thinking-indicator.tsx](../product/ui/src/parts/text-thinking-indicator.tsx), file/export kept) becomes the activity indicator: dots whenever the assistant message is running with no text yet (the old no-pending-tool suppression is dropped — the gap it deferred to no longer has an in-column signal), plus ONE ephemeral line for the latest in-flight tool. The deterministic tool-derived text **complements** (not replaces) the agent-authored D.10 `<fyi>` channel; both share a single display slot arbitrated over the existing [fyi-channel.ts](../product/ui/src/parts/fyi-channel.ts) pub/sub: a new in-flight tool emits `tool-status` (fyi fades), `fyi-appeared` suppresses the indicator's tool text keyed to the current toolCallId (next tool signal lifts it), and text arrival clears everything. Ephemeral by Alastair's verbatim acceptance: any new update replaces the old; only ever one statement beside the dots. Chosen semantics: tool completes with no text yet → line drops, dots stay (the line is state-derived — it reflects what IS in flight); after an fyi auto-fades, suppressed tool text does NOT resurrect (signals don't come back; re-showing stale copy mid-call flickers for no information gain).
**Swap cost**: Low. The arbitration is one event type + two listeners; removing the line reverts one component. Arg-aware messages ("Searching kayaking trips in Aysén…") are a cheap future enhancement — the `input` is already on the client.
...
**Decided**: 2026-06-11 (plan ratified in the goofy-goldstine HITL session) → **2026-06-11 (built)**
**Owner**: activity-status execution session ([planning/03-exec-crosscut-goofy-goldstine-activity-status.md](03-exec-crosscut-goofy-goldstine-activity-status.md) §2.1)
**Rationale**: Copy is content, not code (G.11). The friendly present-progressive per-tool lines live in [cms/ui/tool-status.en.json](../product/cms/ui/tool-status.en.json) — a new `cms/ui/` sibling of `cms/errors/en.json`, loaded the same way (Vite JSON import in the consuming component, `getToolStatusCopy()` mirroring `getToolErrorCopy()`). Unknown tool names fall back to `_default` ("Looking that up…"), so the file ships entries for `show_options` / `get_pricing` ahead of the sibling plans that add those tools and stays correct regardless of merge order. Schema test asserts `_default` presence and every value non-empty; the lookup skips meta (`$…`) and underscore keys so authoring notes can never surface as UI copy. Starter copy is draft — editorial pass is Alastair's, post-merge.
**Swap cost**: Trivial. Editing copy is a JSON edit; renaming/moving the file is one import path + the `$schema-notes` pointer.

## C.tip-1 — `find_tips` ships as the 9th MCP tool: a second shape of the Inform job (traveller wisdom), separate from `lookup`

**Decided**: 2026-05-27 (HITL-ratified) → **2026-06-01 (built)**
**Owner**: find-tips worktree session ([planning/03-exec-customer-tips-tool.md](03-exec-customer-tips-tool.md) — HITL-ratified 2026-05-27, executed 2026-06-01)
**Rationale**: The Inform job has two shapes that read differently and must attribute differently. `lookup` returns **Swoop-authored canon** (`faqitem → inform_chunk`) the agent quotes as "Swoop's guide says…"; `find_tips` returns **traveller-sourced practical wisdom** (`customertip → customer_tip`) the agent attributes as "travellers who've done the W often say…". Granularity differs (1–3 *paragraphs* vs 1–3 *sentences* — the tip is the unit, no aggregation), and provenance differs (canonical vs anecdote) — Sonnet needs to know which it's holding to attribute correctly. Per C.25 tools are named after the **conversational moment**, not the data; practical-wisdom-from-prior-travellers is its own moment (a visitor mid-flow asks "how do I handle the wind?"). It earns its own tool rather than widening `lookup`. Explicitly NOT merged into `customer_story`/`find_someone_who` (the Mirror job) per Alastair 2026-05-27: tips aren't persona-shaped, and forcing them into per-reviewer aggregation would either pollute Mirror retrieval or lose per-tip granularity. `find_tips` inserts after `find_options`, before `illustrate` — the eight-tool surface becomes nine.
**Swap cost**: Low to remove (it's a tool *addition*, blocks nothing downstream): drop the registration, the `customer_tip` derived table, and the ETL/enrich pass. Re-widening `lookup` to absorb tips later would resurrect the voice-attribution muddle this separation exists to avoid.

## C.tip-2 — Derived-table-only (no direct source-mirror query); hybrid RRF retrieval, per-call default of 4

**Decided**: 2026-05-27 (HITL-ratified) → **2026-06-01 (built)**
**Owner**: find-tips worktree session ([planning/03-exec-customer-tips-tool.md §"Open questions" #3/#6/#7](03-exec-customer-tips-tool.md))
**Rationale**: `find_tips` queries a derived `customer_tip` table (embedding + tsv + topic_tags + region + content_hash), never the raw source mirror — identical pattern to the other content tools, so the embedding/idempotency/cache machinery is reused wholesale rather than special-cased. Retrieval is **hybrid**: pgvector halfvec(3072) cosine over `embedding` + tsvector GIN over `tsv`, fused via RRF k=60 through the shared `buildHybridSearchSql` — no bespoke mechanism (`find_someone_who`'s little sister). Topic_tags are **not** a hard filter: relevance comes from cosine alignment, not structured narrowing ("surface the data, let retrieval reason" — the correct default for a small, fuzzy corpus). Per-call default result count is **4** (max 6), matching the agent's "weave 2–4 tips into one utterance" usage.
**Swap cost**: Low. Direct-source-query would mean embedding at query time (latency + cost) — rejected. Pure-cosine (dropping the tsv arm) is a one-template-field change. Changing the default count is a schema default edit.

## C.tip-3 — Eight-topic fixed taxonomy, Haiku-classified at ETL; optional region filter keeps region-agnostic tips eligible

> **SUPERSEDED 2026-06-01 by [C.tip-5](#ctip-5--drop-topic_tags-entirely-find_tips-retrieves-on-embeddings-classifier-stripped-to-region-only).** The topic-tag half of this decision is withdrawn — `topic_tags` did no retrieval work and was removed. The **optional region filter** described here survives unchanged; the classifier was stripped to region-only.

**Decided**: 2026-05-27 (HITL-ratified) → **2026-06-01 (built)**
**Owner**: find-tips worktree session ([planning/03-exec-customer-tips-tool.md §"Open questions" #5](03-exec-customer-tips-tool.md))
**Rationale**: Each tip is tagged at ETL with 0–N topics from a fixed 8-topic taxonomy — **packing / weather / money / safety / transit / food / accommodation / etiquette** — by a Haiku classifier (`cms/prompts/etl/tip-topic/prompt.md`, version 1, temp 0.0), reusing the C.t3a Haiku-ETL-classifier pattern. Zero-topic tips are correct, not failures (a purely emotional tip carries `topic_tags: []`). The one structured narrowing applied is an **optional** region filter: `region = $r OR region IS NULL` — a region-specific ask still sees the large pool of region-agnostic tips, never excludes them. `region` is populated only where a tip names a clear Patagonian sub-region; most tips are region-agnostic (NULL). The `excludeIds` anti-repetition input is wired from inception (forward-coupled to C.anti-rep-*); `customer_tip.id` is INTEGER, so the exclusion casts `::int[]`, not `::uuid[]`.
**Swap cost**: Low. The taxonomy is a `z.enum` in `enrich/schemas.ts` + the classifier prompt — re-classification is an idempotent re-run at a bumped prompt frontmatter version. Region-filter removal is deleting one clause. Adding a topic is additive (enum + prompt + re-classify).

## C.tip-4 — Naming asymmetry: `customer_tip` (singular) vs `customer_story`; prose-only at ship

**Decided**: 2026-05-27 (HITL-ratified) → **2026-06-01 (built)**
**Owner**: find-tips worktree session ([planning/03-exec-customer-tips-tool.md §"Open questions" #9, §"Naming note"](03-exec-customer-tips-tool.md))
**Rationale**: The derived table is `customer_tip` (singular), `source_provenance = 'customertip'` matching the source table name — deliberately asymmetric with the Mirror corpus's `customer_story` (the two serve different jobs and shouldn't be confused by name). `find_tips` ships **prose-only** — no widget. Tips are 1–3 sentences the agent weaves into `<utter>` with traveller-voice attribution; a card surface adds rendering machinery for no conversational gain at v1 (widget revisitable post-Puma if signal warrants). `author_name` (TRIMmed, tab-stripped) is carried for attribution where present ("as Daniel put it…"), falling back to anonymous traveller-voice where NULL. Corpus size at ship is **47 tips** — sparser than the 119 the junction-count implied, but clean editorial selections (R5 partially materialised; quality gate is the live-data HITL sample read).
**Swap cost**: Low. Adding a widget later is additive (a `cms/prompts/tools/find_tips` render path + a UI component); the prose path stays as the fallback. Renaming the table would be a migration + reference sweep.

## C.tip-5 — Drop `topic_tags` entirely; `find_tips` retrieves on embeddings; classifier stripped to region-only

> **UPDATED 2026-06-01 by [C.tip-6](#ctip-6--retire-the-region-classifier-too-no-classify-pass-for-customer_tip-region-column-kept-nullable).** The "classifier stripped to region-only" half of this decision was carried only a few hours: on the same review Al asked why we tag *at all*, and we retired the region classifier entirely too. The `region` **column + soft filter survive**; what's gone is the Haiku pass that *inferred* region. Everything else in C.tip-5 stands.

**Decided**: 2026-06-01 (HITL, live-data sample review) — supersedes the topic-tag half of [C.tip-3](#ctip-3--eight-topic-fixed-taxonomy-haiku-classified-at-etl-optional-region-filter-keeps-region-agnostic-tips-eligible)
**Owner**: find-tips worktree session
**Rationale**: Reviewing the 45-row live corpus, Al questioned why we tag at all when retrieval moved to embeddings. Inspection confirmed it: `find_tips` retrieves **purely** via hybrid search (content embedding `<=>` + `ts_rank`, fused RRF k=60) — [`find-customer-tips.ts`](../product/connector/src/data/find-customer-tips.ts) never filtered or ranked on `topic_tags`. The 8-topic taxonomy was a vestige of an earlier filter-based draft; the tags were `SELECT`-ed and returned as metadata but did zero retrieval work. For a small, fuzzy corpus the tip text itself is the topic signal, and the agent reads all 45 tips in full anyway — so a coarse label added a Haiku classify pass, an enum to maintain, and the recurring "is hiking/culture missing?" maintenance tax, for no lift. **Cut**: the `topic_tags` column (migration [014](../product/connector/migrations/014_customer_tip_drop_topic_tags.sql)), its GIN index, the `TIP_TOPICS` enum, `topicTags` from `CustomerTip{,Public}Schema`, and the topic half of the classifier. **Kept**: the optional region filter (it is the one structured narrowing and is genuinely load-bearing for region-specific asks). Because the same Haiku pass populated *both* tags and region, it was **stripped to region-only** and renamed `tip-topic` → `tip-region` (classifier file, prompt dir `cms/prompts/etl/tip-region/`, `TipRegionOutputSchema`, `classify:tip-region` source key). `inform_chunk`'s separate `topic_tags` (the `lookup` tool) is **untouched** — different tool, different decision.
**Swap cost**: Medium to revive. Re-adding tags means a new column + migration, re-introducing the enum + a classify pass, and re-threading `topicTags` through the schema/data/tool/tests. But there's no reason to: if a future UI wants topic chips, derive them at render time or revisit then. Region-only classify keeps the Haiku dependency in the ingest path for one field — acceptable; it is the only structured retrieval lever.

## C.tip-6 — Retire the region classifier too: no classify pass for `customer_tip`; region column kept nullable

**Decided**: 2026-06-01 (HITL, same live-data review as [C.tip-5](#ctip-5--drop-topic_tags-entirely-find_tips-retrieves-on-embeddings-classifier-stripped-to-region-only)) — extends C.tip-5
**Owner**: find-tips worktree session
**Rationale**: Having stripped the classifier to region-only (C.tip-5), Al pressed the same question one level deeper: "why are we tagging at all — I thought we'd moved to embeddings?" The region pass didn't survive the scrutiny. Three facts settled it: (1) `find_tips` retrieval is **100% hybrid** (content embedding + tsv RRF) — `region` is a *query-time* filter the agent optionally supplies, never a ranking input; (2) the source `customertip` record carries **no region/location field** (only `id / content / name / created / deleted`), so the pass could only ever *infer* a region from the tip text — a label the content embedding already encodes (a query about "money in El Calafate" retrieves the El Calafate tip via cosine regardless); (3) on the 45-row corpus the pass fired on just **8 rows**, all places the text named explicitly. So an LLM inference pass per row — a prompt, a cost label, and a `classified_at` idempotency gate to maintain — bought nothing retrieval couldn't already do. This is the same "prefer conversational reasoning / embeddings over programmatic tagging on small fuzzy data" call that killed `topic_tags`, applied consistently. **Cut**: the `tip-region` classifier (`classify/tip-region.ts`), its prompt dir `cms/prompts/etl/tip-region/`, `TipRegionOutputSchema` + the `CLASSIFIER_SCHEMAS['tip-region']` entry, the `haiku:tip_region` cost label, the `classify:tip-region` run wiring, and the now-vestigial `customer_tip.classified_at` column (migration [015](../product/connector/migrations/015_customer_tip_drop_classified_at.sql)). **Kept**: the `region` **column** (nullable) and the **soft query filter** `region = $r OR region IS NULL` in [`find-customer-tips.ts`](../product/connector/src/data/find-customer-tips.ts) — `region` is a genuine cross-corpus dimension (`find_options` / `find_someone_who` / `find_inspiring` all expose the same agent-supplied filter), so tips being region-aware is consistent, not anomalous; it simply stays null until/unless source data ever carries a real location. Net: `customer_tip` has **no classify pass at all** — only the embed-derived pass (embedding + tsv). `inform_chunk`'s separate tags remain untouched (different tool).
**Swap cost**: Low to revive *inference* (re-add a classifier file + prompt + cost label + run wiring + re-add `classified_at` via migration). But the better revival, if region ever matters more, is to **map it from source** deterministically rather than infer it with an LLM. The `region` column and filter staying in place means that revival is data-only — no schema or tool change.

## C.anti-rep-1 — Anti-repetition is server-side, per-session, default-on for all content types except trip + tour

**Decided**: 2026-05-27
**Owner**: agent-a9759bd6decb33791-antirep ([planning/03-exec-crosscut-anti-repetition.md](03-exec-crosscut-anti-repetition.md) — HITL-ratified 2026-05-27)
**Rationale**: Repeating the same `inspire_passage`, `customer_story`, `trust_proof`, `inform_chunk`, or `image` two turns in a row reads as the agent not paying attention. The agent's previous lever was the (now superseded as sole producer) `find_options.exclude` field per [C.focused-shamir-5 — `find_options` accepts an agent-supplied `exclude` list to omit cards across turns](decisions.md), which put the burden on Sonnet to remember everything it had ever shown. The new default: the orchestrator computes the exclude list from `SessionState.seenItems` on every dedup-eligible tool call. **Carve-out**: `trip_card` and `tour_card` rows are NEVER added to the seen-set — Swoop is selling trips and tours; repeating an option is the work, not a bug. The agent's existing `find_options.exclude` lever is preserved as an additive override (can ADD entries the orchestrator wouldn't have, never SUBTRACT).
**Swap cost**: Low. The wire layer is the existing per-primitive `excludeIds` / `excludeCanonicalUrls` parameter shape generalised across every primitive. To undo: stop computing excludes orchestrator-side (the parameters become no-ops); session state would carry an unused `seenItems` field. To re-add trip/tour tracking: extend the `SeenItems` schema with `trip` / `tour` keys — the carve-out is structural (absence from the schema), not a conditional.

## C.anti-rep-2 — Seen-set lives in ADK session state; no new datastore, no TTL, session-scoped lifetime

**Decided**: 2026-05-27
**Owner**: agent-a9759bd6decb33791-antirep ([planning/03-exec-crosscut-anti-repetition.md](03-exec-crosscut-anti-repetition.md) §2.1 + HITL Q4)
**Rationale**: `SessionState.seenItems` is a per-type record of arrays of strings (uuids / canonical URLs / stringified integer ids). Lives in the orchestrator's `SessionStore` — same surface that already owns conversation history, triage, consent, wishlist. No new Postgres table. No Redis. No in-process Map. ADK's session lifecycle owns cleanup; when the session ends the seen-set evaporates with it. **No TTL within the session**: once shown, never shown again in the same session. If a real need for "let me revisit X" emerges, it's an agent action (Sonnet calls a tool with an explicit override or asks the visitor's permission), not a passive timeout. Same architectural posture as [B.22 — post-M4 Postgres-backed ADK session store](decisions.md): when session state moves to Postgres, `seenItems` rides through unchanged.
**Swap cost**: Trivial structurally — the schema field is optional + defaulted to empty arrays, so older sessions round-trip cleanly. Removing the feature would mean stripping the read+write pair from `invokeTool`; the field on `SessionState` would become unused data carried for no reason.

## C.anti-rep-3 — Mark-as-shown happens at the orchestrator boundary on every tool return; the agent has no role

**Decided**: 2026-05-27
**Owner**: agent-a9759bd6decb33791-antirep ([planning/03-exec-crosscut-anti-repetition.md](03-exec-crosscut-anti-repetition.md) §2.3 + HITL Q6)
**Rationale**: Every item the connector returns from a tracked tool gets marked shown — the visitor's reality is "whatever the connector returned", not "what the agent decided to surface". Embedded images count: when an `inspire_passage` or `customer_story` carries an `image.canonicalUrl`, the image URL is marked shown alongside the parent row (a subsequent `illustrate` call therefore cannot return that picture again). Same rule for the embedded image on a trip/tour/hotel/region_base card in `find_options` — the IMAGE is marked even though the row itself (for trip/tour) is in the carve-out. Aligns with Luke's parked right-panel UI where cards + images move to the right pane and the agent doesn't pick what to display — "marked as shown" cleanly equals "everything the tool returned".
**Swap cost**: Low. `extractSeenDelta` in `product/orchestrator/src/connector/anti-repetition.ts` is the one place the marking logic lives — switching to agent-selected marking would mean parameterising it on a list the agent passes back, rather than reading the structured result.

## C.anti-rep-4 — Wire shape: existing per-primitive `excludeIds` parameter, generalised across every dedup-eligible primitive

**Decided**: 2026-05-27
**Owner**: agent-a9759bd6decb33791-antirep ([planning/03-exec-crosscut-anti-repetition.md](03-exec-crosscut-anti-repetition.md) §2.4 + HITL Q1/Q2/Q7)
**Rationale**: The HITL ratification rejected the elaborate `__seenItems` / `__newlySeen` envelope convention sketched in the DRAFT plan in favour of the simpler "regular tool-call arguments" pattern. The existing `excludeIds: number[]` parameter from [C.focused-shamir-5 — `find_options` accepts an agent-supplied `exclude` list to omit cards across turns](decisions.md) is the foundational mechanism; AntiRepetition extends the same shape to every primitive whose tool surfaces dedup-eligible content (`findInspirePassages`, `findCustomerStoriesByPersonaSignal`, `findTrustProofsByConcern`, `findInformChunksByQuestion`, `findImagesByKeywords`). Two parameter shapes: `excludeIds?: string[]` (uuid for content primitives) or `number[]` (integer for hotel/region_base — unchanged from C.focused-shamir-5), and `excludeCanonicalUrls?: string[]` for image-keyed types (per C.anti-rep-5). The connector stays purely stateless — handlers receive validated tool inputs and return validated tool outputs; nothing more. Orchestrator-side, `invokeTool` brackets every dispatch with a `sessionStore.get` (compute excludes) + `sessionStore.update` (merge returned ids/URLs). ADK's `FunctionTool.execute` callback receives a `tool_context: Context` whose `sessionId` getter (inherited from `ReadonlyContext`) feeds the bracketing.
**Swap cost**: Low — the wire layer is just optional parameters on existing schemas; removing them breaks no callers. The orchestrator's bracketing is in one file (`product/orchestrator/src/connector/anti-repetition.ts` + the `invokeTool` deps).

## C.anti-rep-5 — Image dedup key is `canonical_url`, not `image.id`; same principle for web-page references

**Decided**: 2026-05-27
**Owner**: agent-a9759bd6decb33791-antirep ([planning/03-exec-crosscut-anti-repetition.md](03-exec-crosscut-anti-repetition.md) §2.6 + HITL Q5)
**Rationale**: "Never show the same picture twice" is the user-correct rule, and source-of-truth identity for the visitor's reality is the rendered URL, not the database row. The corpus already carries multiple `image` rows pointing at the same imgix URL (the 3-kayak case observed in dev — distinct rows, identical content); treating them as one to the visitor's eye is the right call. Same principle applies to any future URL-bearing type (`blog_post`, FAQ pages, external references): dedup by canonical URL rather than by row id. Implication: `SeenItems.image` is a `string[]` of canonical URLs (not stringified integer ids), and image-bearing primitives accept `excludeCanonicalUrls?: string[]` rather than an id list. UUID-keyed types (inspire_passage, customer_story, trust_proof, inform_chunk) keep their natural uuid key; integer-keyed types (hotel, region_base) keep their stringified integer id.
**Swap cost**: Negligible — the choice is encoded in a single column reference in `findImagesByKeywords` (`canonical_url <> ALL($N::text[])`) and in the projection logic in `findInspirePassages` / `findCustomerStoriesByPersonaSignal` that blanks an embedded image when its URL is in the seen-set. Switching to id-keyed dedup would require restoring `image_id`-based lookups; the schema field type would change from `string[]` to `string[]` of stringified ids — same shape, different content.

## C.focused-shamir-6 — Tour region is informational, not a filter; derived from the `page.parent_id` chain

**Decided**: 2026-05-18
**Owner**: focused-shamir-52524c session ([planning/03-exec-crosscut-find-options-v2-backfill.md](03-exec-crosscut-find-options-v2-backfill.md) — late follow-up)
**Rationale**: C.focused-shamir-2 shipped with `tour_card.region` always NULL and a known limitation that region-filtered tour queries returned zero. Two follow-up problems surfaced together: (1) region IS recoverable from the source — a recursive walk up `page.parent_id` finds ancestors of pagetype `National Park` / `Region` / `Region-Activity` for some tours (e.g. tour 72 → "Atacama Desert" via depth 1, tour 77 → "Torres del Paine National Park" via depth 3); and (2) even with region populated, a flat ILIKE filter loses hierarchy (Torres del Paine ⊂ Patagonia ⊂ Chile — a TdP-tagged tour should surface for "Patagonia" queries but a single-string match can't encode that). Solution: derive region informationally, surface it on each card, but **remove the region filter entirely** for tours. `queryTourCardsByFilter` returns every tour regardless of the visitor's `region` value; the conversational agent reads each card's `region` field (when present) and frames contextually in prose. Tours with NULL region are deliberately pan-region (the page chain rolls up through general Swoop/Patagonia parents with no Region-shaped ancestor — that's editorial intent, not a data gap). The find_options prompt was updated to make this explicit. NULL-region semantics here are intentionally asymmetric with trip/hotel/region_base, where NULL means "couldn't derive" (data gap).
**Swap cost**: Low for the derivation (recursive CTE is self-contained in `composeTourCard` SQL). Low for the filter removal (deleting the `region ILIKE` clause). Re-adding region filtering later would need either a hierarchy model (region_path string or regions[] array) or accepting the flat-ILIKE limitation; recorded as a future option if tour catalogue grows substantially. Cache made re-compose free — zero Gemini tokens spent on the recompute (verified 2026-05-18).

## C.focused-shamir-5 — `find_options` accepts an agent-supplied `exclude` list to omit cards across turns

**Decided**: 2026-05-15
**Owner**: focused-shamir-52524c session ([planning/03-exec-crosscut-find-options-v2-backfill.md](03-exec-crosscut-find-options-v2-backfill.md) §2.5.b)
**Rationale**: `find_options` had no session-state tracker — same filter + same data → same cards every turn. Lightweight middle path between pure-random (C.focused-shamir-4) and a full `SessionState.shownCards` tracker: the agent passes `exclude: Array<{type, id}>` on each call to omit cards it doesn't want repeated (cards it surfaced earlier, options the visitor explicitly rejected, deliberate upsell rotation). The connector remains stateless; Sonnet owns its own shown-history (which it has in context anyway). Each primitive's `Options` interface gained `excludeIds?: number[]`; SQL applies `AND id <> ALL($N::int[])` (empty-safe). `find_options.ts` dispatch splits the exclude list by type so a trip-id 369 doesn't accidentally exclude a tour id 369 (distinct id-spaces).
**Swap cost**: Trivial. The field is optional on the schema; removing it deprecates a feature but breaks nothing structural. Full `SessionState.shownCards` tracker stays deferred — revisit only if random + agent-exclude prove insufficient under live use.

## C.focused-shamir-4 — All four `find_options` primitives use `ORDER BY RANDOM(), id`

**Decided**: 2026-05-15
**Owner**: focused-shamir-52524c session ([planning/03-exec-crosscut-find-options-v2-backfill.md](03-exec-crosscut-find-options-v2-backfill.md) §2.5.a)
**Rationale**: Today's deterministic ordering on each primitive meant same-filter calls returned identical card sets every time. `find_options`'s default is imagination-stoking *variety*, not deterministic ranking. All four card-query primitives switch to `ORDER BY RANDOM(), <id-column>` (id as a stable tiebreaker). Supersedes the implicit ranking semantics in `queryTripCardsByFilter` (shortest+cheapest first), `queryHotelCardsByFilter` (cheapest first), and `queryRegionBaseCardsByFilter` (most-popular bases first). `queryTourCardsByFilter` was born random. `find_options` is *pure structured filter* (per C.t4) — it doesn't accept free-text hints; embedding-ranked retrieval lives in sibling tools (`find_inspiring`, `find_someone_who`). The `_card.embedding` columns exist for cross-tool reuse, not as a `find_options` ranking layer. Session-scoped shown-card tracking (Option C from the design conversation) deferred — revisit if random + agent-exclude isn't enough.
**Swap cost**: Low. Existing primitive tests had no ordering assertions to update (verified). Tests assert count + shape + member-of-set, which is the right shape for variety-driven tooling anyway. If pseudo-determinism for testing becomes a friction later, swap to app-side shuffle with optional seed.

## C.focused-shamir-3 — `find_options.blendCards` is a four-way even split across `trip | tour | hotel | region_base`, extras to trips

**Decided**: 2026-05-15
**Owner**: focused-shamir-52524c session ([planning/03-exec-crosscut-find-options-v2-backfill.md](03-exec-crosscut-find-options-v2-backfill.md) §2.4)
**Rationale**: The v3 ratio (C.bf-3: 2 trips + 1 hotel + 1 region_base at limit=4) was trip-heavy and missed tours. With tour-v2 going live (C.focused-shamir-2), the default mix becomes a four-way even split: `base = floor(limit/4)`, extras to trips. At the default `limit=4`: 1 of each variant — trip + tour + hotel + region_base. At `limit=5`: 2+1+1+1. At `limit<4`: drops to trips-only (Sonnet's default is 4 so this is rare). Framing: `find_options`'s default is imagination-stoking *variety*, not population-weighted fairness or single-type dominance. Deficit redistribution stays as "top up via trips" (most populous source). Supersedes C.bf-3.
**Swap cost**: Low. Ratios are local to one function; reversible by editing the quota computation.

## C.focused-shamir-2 — Tours go live; `queryTourCardsByFilter` + `tour_card` derived table replace the C.bf-6 trip fallback

**Decided**: 2026-05-15
**Owner**: focused-shamir-52524c session ([planning/03-exec-crosscut-find-options-v2-backfill.md](03-exec-crosscut-find-options-v2-backfill.md) §§2.1–2.4)
**Rationale**: With the `tour` table now populated (C.focused-shamir-1) and the embedding-cache architecture (C.embedding-cache-1) making derived-table re-compose cheap, the v2 tranche of the find_options polymorphism roadmap can land. New `tour_card` derived table (migration 011) mirrors `trip_card` with three intentional shifts: `day_count` as the honest day-signal (no `tour.duration` in source), future-proofed but always-NULL `from_price` / `currency_code` / `group_size_max` / `accommodation_style` columns (no source data today), and `halfvec(3072)` embedding from inception (post-C.46 / migration 009). `composeTourCard` derives identity from the parent contentblock's page (extends the C.focused-shamir-1 pattern), with image fallback to `page.image_id` for the 3 of 11 tours lacking `tour.image_id`. `queryTourCardsByFilter` lives in the connector data layer with the same shape as the trip/hotel/region_base primitives. `find_options.ts` `case 'tour'` switches from `queryTripCardsByFilter` (the C.bf-6 fallback) to `queryTourCardsByFilter`. Result: `puma_dev` carries 11 tour_cards, all with embeddings, all retrievable via `find_options(preferredType: 'tour')`. Supersedes C.bf-6.
**Known limitation today (region NULL)**: `tour_card.region` is NULL on every row. The candidate source — `contentblock.region_id` — turned out to be a dangling reference (every tour contentblock has `region_id = 36`, but `area.id = 36` doesn't exist). `page.ntag_ids` is empty on tour pages (the ETL only aggregates `ntags_lookup` for image/trip/contentblock/video, not page). The practical impact is small (all 11 tours are Patagonia-themed; visitors who type "Patagonia tours" with a region filter get zero results, so the find_options prompt now nudges Sonnet to drop the region filter when `preferredType: 'tour'`). Recovery options are sized in `inbox.md`; revisit if tour catalogue grows beyond Patagonia.
**Swap cost**: Medium. The data primitive + dispatch swap is local; reverting to the trip fallback is a one-line swap in `find_options.ts:case 'tour'`. `tour_card` table is forward-only (C.31). Future Swoop population of `tour.from_price` / `duration_days` / `group_size_max` would surface automatically through `composeTourCard`'s already-wired columns; no further code change.
...
**Decided**: 2026-05-14
**Owner**: focused-shamir-52524c session ([planning/03-exec-c-t3.md](03-exec-c-t3.md) 2026-05-14 addendum)
**Rationale**: `puma_dev` had zero tours. C.t3's original `transformTour` filtered on `tours.title` being non-null — but that column is empty (NULL or `''`) on every one of the 15 source rows; it is vestigial in Swoop's CMS. A tour's real identity lives on the page its contentblock belongs to (`tours.content_block_id → contentblock.page_id → page.{title,alias,canonical_url}`). The all-null count had been rationalised into the ETL runbook as expected output rather than treated as the bug it was. Fix: `transformTour` resolves title/slug/canonical_url/page_id from the page via a new `Lookups.contentblockById` map, and filters to itinerary-type contentblocks (`contentblock.type_id = 152` — 12 of 15 `tours` rows; the other 3 hang off a hotel / guidebook / "Swoop Says" block). Tours whose page was filtered upstream (test / profile / deleted) drop cleanly. Result: `puma_dev` now carries 11 tours + 35 tour_items, and the 3 not-actually-a-tour rows are correctly excluded.
**Swap cost**: Low for the title-source — it's the documented `page`-as-hub pattern, same as trip / hotel / vessel. The `type_id = 152` discriminator is a magic number: Swoop's dump carries no defining table for contentblock types, so an open question to Swoop engineering (questions.md "Tour content population") asks them to confirm it, with a `page.pagetype = 'Itinerary'` cross-check in the C.t3 verification step as the drift guard. Note: this populates tour *identity* fields only — `from_price`, `duration_days`, `group_size_max`, `region_id` stay null (no source columns), so the `find_options` v2 tour-card tranche (decisions C.50 / C.bf-1 / C.bf-6) is no longer data-*empty* but still lacks commercial fields; whether that un-gates v2 is Al's call.
...
**Decision**: name the carve-out explicitly. `--mode=live` is the supported full-run path. `--mode=batches` is **request-build-verified only** until the submit + poll + result-stream wiring is built. The follow-up task is now queued in [next-steps.md — Chunk C section](../next-steps.md) and the operator-facing gotcha is at [gotchas.md — annotate-images batches](../gotchas.md). The original runbook caveat at [product/docs/ops/image-annotation-rerun.md](../product/docs/ops/image-annotation-rerun.md) stays as the read-this-before-running gate.
...
**Swap cost**: Trivial. One sentence in `product/docs/ops/evalset-growth.md` § "Cadence + ownership" gets updated whenever ownership changes.
...
**Customertip — RESOLVED 2026-06-01.** ~~The 2026-04-30 dump did not include `customertip` (119 expected) or `pressreview`. Al has a separate ask outstanding to Swoop.~~ Swoop delivered `customertip` on 2026-05-27; the schema-lookup (47 live rows, not the 119 the junction-count implied — see [03-exec-customer-tips-tool.md §schema addendum](03-exec-customer-tips-tool.md)) confirmed shape, and the data became a live conversational surface via the **`find_tips`** tool (the 9th MCP tool — a second shape of the Inform job). See decisions C.tip-1…C.tip-4 above and [planning/03-exec-customer-tips-tool.md — customer_tips ingest + `find_tips` tool](03-exec-customer-tips-tool.md) (HITL-ratified 2026-05-27, built 2026-06-01). The customertip half of [questions.md item (a)](../questions.md) is now closed. (`pressreview` remains undelivered and unused — out of scope for Puma.)
