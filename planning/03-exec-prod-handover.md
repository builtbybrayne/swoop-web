...
**Status**: Scope + structure ratified 2026-06-16 (Alastair). The dev handover is **built** at [`../product/handover/`](../product/handover/README.md) — `productionisation.md` + `maintenance.md` drafted for the writable-now sections; the **deploy-topology** section is stubbed pending the M4 deployment-shape decision. A sales handover doc is deferred (the folder is its home). This file is the planning record + the section-source map.
...
**Audience**: Thomas / Richard — competent engineers new to *this* codebase. Role-based, paste-ready, **single-source-of-truth via cross-links** (the same discipline as `docs/ops/README.md` §Conventions — link, don't duplicate; the runbooks/`.env.example`/compliance bundle remain canonical).
...
| Observability: Cloud Logging + Error Reporting + `severity≥ERROR` alert policy | Thomas | APIs + SA `logging.logWriter` + Ops Agent + `EVENT_SINK=cloud-logging` | questions.md (added 2026-06-16) + [docs/ops/observability.md](../product/docs/ops/observability.md) |
...
4. **Database** — Postgres 18 + `pgvector`/`tsvector`/`pg_trgm`; the migration runner + 001–020; populating the derived store (ETL → enrich → embed — pointers to `docs/ops/etl-rerun.md` + `embedding-rerun.md`); the **`pg_dump`→restore promotion path** (laptop→Mini→GCE, per `discoveries.md` 2026-05-21).
...
6. **Observability & ops** — pointer to `docs/ops/` index + `observability.md` (collection/error-surfacing) + the alert policy.
...
`product/orchestrator/.env.example` + `connector/.env.example` · `docs/ops/*` (incl. the new `observability.md` + `demo-server.md`) · `cms/legal/compliance-bundle/*` + `planning/swoop-legal-review-pack.md` · `discoveries.md` (pg_dump recipe, page-as-hub, etc.) · `gotchas.md` (Postgres.app, dotenv override, model IDs) · `questions.md` (the open Swoop asks) · `ui/HANDOVER.md` · migrations `001–020`.
...
**Resolved 2026-06-16 (Alastair):** a dedicated **`product/handover/`** folder (not a single root file); the **dev** handover splits into [`productionisation.md`](../product/handover/productionisation.md) (one-time stand-up) + [`maintenance.md`](../product/handover/maintenance.md) (run & evolve) + a README index; a **sales** handover doc is deferred (the folder is its home); the writable-now sections are drafted now, deploy-topology stubbed pending M4; audience assumed GCP+Node fluent (terse, link-heavy). Original options kept below for the record.

1. **Location / shape** — *Recommend* `product/HANDOVER.md` (whole-product root, sibling-discoverable to `ui/HANDOVER.md`, which it cross-links). Alternatives: a `product/docs/handover/` set, or `planning/`. → **resolved: a `product/handover/` folder.**
