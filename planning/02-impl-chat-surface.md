...
- **D.t9 — Widget replace for intent-named tools** (post-M1, gated on chunk C; revised 2026-04-29): adds new widgets for the eight-tool intent-named surface settled in [02-impl-retrieval-and-data.md](02-impl-retrieval-and-data.md) §2.2 + decision C.25. Likely shapes: a vibe-passage panel for `find_inspiring`, a story-vignette card for `find_someone_who` *(conditional on C.26)*, a proof carousel for `find_proof`, a chunk-list for `lookup`, an options-card-set for `find_options`. The existing 4 PoC widgets from D.t3 partly carry forward: `inspiration` and `lead-capture` survive (rendering `illustrate` and `handoff` respectively); `component-list` and `component-detail` are deprecated alongside their consuming tools (`search` / `get_detail`). This is replace, not augment — earlier (2026-04-28) phrasing of this task as an *augment* is superseded; the 2026-04-29 tool surface deprecates `search`/`get_detail` so their widgets retire too. **Gated on**: C.t2 (new schemas in `@swoop/common`) + C.t4 (real connector outputs the widgets hydrate from). **Pairs with B.t3a** (matching ADK FunctionTool wrappers in chunk B). Each new widget registers via `makeAssistantToolUI` against the new tool name — the registration mechanism itself doesn't change. Estimated 1–2 days mostly mechanical work once the upstream schemas + outputs land.

D.t1–D.t3 are the Phase 1 vertical slice contribution. D.t4–D.t7 come in the Phase 2 fan-out. D.t8 is near-delivery. **D.t9 is post-M1** — runs in parallel with B.t3a once chunk C's tool surface lands.

Estimated: 4–5 days of focused work post-A (once `ts-common` streaming event shape is stable) for D.t1–D.t8. One agent can drive this end-to-end; parallelising D vs B is a natural split once Phase 0 contracts land. D.t9 adds ~1–2 days post-M1.
