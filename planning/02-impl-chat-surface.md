...
- **D.t9 — Widget augment for sales-shaped tools** (post-M1, gated on chunk C): adds new widgets for the 5 new sales-shaped tool outputs introduced in [02-impl-retrieval-and-data.md](02-impl-retrieval-and-data.md) §2.2 — likely a story-vignette card for `recall_someone_who`, a proof carousel for `build_confidence`, an evocative-content panel for `stoke_imagination`, an options-card-set for `offer_options`, and a side-by-side comparison view for `compare_paths`. The existing 4 widgets (`component-list`, `component-detail`, `inspiration`, `lead-capture` from D.t3) keep rendering the existing 5 tools' outputs (`search`, `get_detail`, `illustrate`, `handoff`, `handoff_submit`) — this is augment, not replacement. **Gated on**: C.t2 (new schemas in `@swoop/common`) + C.t4 (real connector outputs the widgets hydrate from). **Pairs with B.t3a** (matching ADK FunctionTool wrappers in chunk B). Each new widget registers via `makeAssistantToolUI` against the new tool name — the registration mechanism itself doesn't change. Estimated 1–2 days mostly mechanical work once the upstream schemas + outputs land.
