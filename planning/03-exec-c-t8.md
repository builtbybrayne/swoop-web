...
**Produces**:
- `handover/ops/etl-rerun.md` — operator runbook for re-running the SQL-dump → Postgres transform when source data changes.
- `handover/ops/embedding-rerun.md` — operator runbook for re-running the embedding + classifier pass.
- `handover/ops/image-annotation.md` — operator runbook for running the image annotation pipeline (cost preview, dry-run, partial-by-tag).
- `handover/ops/migrations.md` — operator runbook for running, rolling forward, and recovering from Postgres migrations (`node-pg-migrate` operational story).
- `handover/ops/troubleshooting.md` — symptom-driven recovery guide spanning all four runbooks above.
- `handover/ops/README.md` — index linking the runbooks (existing file already lists `evalset-growth.md`; append the new four).
...
- **The runbooks are content** (G.11). They live in `handover/ops/` with the existing `evalset-growth.md`. Iteration is content work — Swoop's team can edit them post-handover.
...
The companion runbook for context: `handover/ops/evalset-growth.md` (shipped 2026-04-29 per H.t7). It's the tonal benchmark — operator-task ordering, visible-from-the-top-of-the-page commands, *"open this alongside the runbook in a tab"* pragmatism. Don't overthink the C.t8 set; match its shape and tone.
...
- [`handover/ops/evalset-growth.md`](../handover/ops/evalset-growth.md) — tone/shape reference. Match this.
...
Append to the existing `handover/ops/README.md` — the new five entries listed alongside `evalset-growth.md`. Each entry is one line: filename + one-sentence purpose + a *"start here when…"* trigger.
...
|---|---|---|
| ETL re-run runbook | `handover/ops/etl-rerun.md` | New |
| Embedding re-run runbook | `handover/ops/embedding-rerun.md` | New |
| Image annotation runbook | `handover/ops/image-annotation.md` | New |
| Migrations runbook | `handover/ops/migrations.md` | New |
| Troubleshooting runbook | `handover/ops/troubleshooting.md` | New |
| Index | `handover/ops/README.md` | Existing — append |
| Tone benchmark | `handover/ops/evalset-growth.md` | Existing — read-only here |
...
3. `handover/ops/README.md` lists all six runbooks (the four new task-specific + the new troubleshooting + the existing evalset-growth).
...
- **Future runbooks** — the convention C.t8 sets (`handover/ops/<topic>.md`, operator-task-shaped, failure-mode appendix, open-items-at-bottom) is the convention for any future runbook (e.g. orchestrator deploy runbook, monitoring-alert runbook).
...
**Files landed** (all under `handover/ops/`):
...
**Verification**:
- All six runbook files at `handover/ops/` plus the README index — confirmed via `ls`.
...
- Voice-coherent with `evalset-growth.md` already at `handover/ops/`. Same role-based framing.
