...
> **⚠ Binding GDPR commitment (2026-06-16):** Puma's tier-1 consent copy publicly tells visitors *"any saved data is always anonymised before we do any internal analyses."* So this harness's **first step — before any expert prompt touches a transcript — must be a de-identification sweep** (strip direct identifiers: name, email, phone, contact). Analysing raw visitor PII would breach the disclosed promise. "Anonymised" is the visitor-facing term (Alastair's call — more publicly recognised); the mechanism is realistically pseudonymisation; counsel confirms the final legal term. Recorded in [swoop-legal-review-pack.md](swoop-legal-review-pack.md) D-3.1.9. NB no retention *window* for analysis data is implemented or committed yet — keep public commitments loose.
