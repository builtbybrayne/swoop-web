...
- **C.t8** (runbooks): the Cloud Run Job invocation pattern, the cost-cap env var, the rate-limit tuning notes, and the prompt-iteration loop all become C.t8 documentation. C.t3a leaves stub notes in `product/cms/ops/`; C.t8 fleshes them out.
