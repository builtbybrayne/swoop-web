...
**Next, in order:**
1. **T3-5 voice (Alastair, G.7)** — replace two `TODO(T3-5)` placeholder headers with edited copy, preserving their load-bearing structural signals: (a) the memory-mode wrapper in `product/orchestrator/src/agent/memory-agent.ts` (confirm-before-save / content-hygiene / `finish_memory`); (b) the `AUTHORITATIVE_HEADER` in `product/orchestrator/src/agent/sales-memory-loader.ts` (authoritative / state-as-fact / dated-for-staleness).
...
**Fix queue status (late evening — F1–F4 EXECUTED with per-instance gos; commits `728a172` `55fd6a0` `01a2d5e` `980a4d6`, NOT pushed)**: F1 sibling traps ✅ (live-proven: tour+kayaking 0→4 cards, trip+lodge 0→4) · F2 lookup URLs ✅ (ops O0→O3 ran: baseline dump at `data/backups/`, migration 018, ETL faqset rescue — 183 FAQ blocks were hiding in `contentblock_navigationcard` — and a £0 cache-hydrated re-compose; `inform_chunk` URL coverage 18→890; the lookup widget rendered on FAQ content for the first time) · F3 hooks ✅ console clean · F4 description ✅ · F5 Mini refresh ⚠ open — needs Alastair (no SSH; fixes need pushing first). **F6 ✅ EXECUTED** (same evening, "f6 go"): persona-summary classifier ran sync — 953/953 rich personas now populate `customer_story.persona_summary` + fresh persona_embeddings (£1.46, 9.7 min); find_someone_who matches on real persona semantics for the first time. See diagnosis §6a for the run record + the five-key-collision footnote.
...
2. ~~**B.t9 smoke test**~~ — **landed 2026-05-18** via [03-exec-agent-runtime-t9.md — B.t9 ADK skill-loader integration](planning/03-exec-agent-runtime-t9.md). Orchestrator now boots with `[orchestrator] loaded 14 skills from .../cms/prompts/skills: arrived-with-ai-itinerary, engaging-a-browser, engaging-a-dreamer, engaging-a-planner, engaging-a-skeptic, group-tour-surfacing-for-solos, pattern-anniversary-couple, pattern-budget-solo-traveller, pattern-gauchos-and-estancias, pattern-overwhelmed-researcher, pattern-puma-photographer, pattern-w-vs-o-wrestler, tailor-made-prospect-posture, triage-to-referral`. Skills are wrapped in a `SkillToolset` alongside the 8 connector FunctionTools and attached as the single entry in `LlmAgent.tools`. Behavioural trigger-firing (description-match → skill loads at runtime) is the next live-traffic observation, not a deterministic smoke test.
...
7. ~~**C.t8** — ETL + annotation runbooks~~ ✅ **done 2026-05-02.** Six runbooks at `product/docs/ops/`: `etl-rerun.md`, `embedding-rerun.md`, `image-annotation-rerun.md`, `migration-management.md`, `prompt-version-rollback.md`, `troubleshooting.md`. Plus index `README.md`.
