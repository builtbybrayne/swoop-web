...
- **C.t6** ✅ live-mode code done 2026-05-02; ⏳ **annotation run still pending operationally**; ⏳ **`--mode=batches` submission wiring deferred** (request-build verified but the POST + poll + result-stream isn't implemented — see [decisions.md C.52 — image-annotation `--mode=batches` submission is a deliberate C.t6 scope-cut](planning/decisions.md) + the gotcha at [gotchas.md — `annotate-images --mode=batches` builds the payload then bails](gotchas.md)). Folded with C.t3a's image-annotation pass per decision C.40 — one Claude Vision call produces description + annotation + 4 tag arrays. **Pickup task** (~1–2 hrs TDD): copy the `BatchClient` pattern from [product/ingestion/src/enrich/anthropic-batch-client.ts](product/ingestion/src/enrich/anthropic-batch-client.ts) (C.t10's Haiku batches client) into a `VisionBatchClient` in `product/ingestion/src/images/`, wire to `runBatches`. Saves ~$17 USD / £13 on each full image-annotation re-run vs `--mode=live`.
